...
def validate_path(path: str) -> tuple[bool, str]:
    ...
    if p.suffix.lower() != ".png":
        return False, f"file extension must be .png, got '{p.suffix}'"

    # Need at least: {letter}/{publisher}/{app}.png
    if len(parts) < 3:
        return (
            False,
            "path must have at least 3 components, "
            "e.g. 'm/microsoft/teams.png'",
        )

    first_dir = parts[0]
    publisher = parts[1]
    filename_stem = p.stem  # without .png
...
def validate_image(path: str) -> bool:
    """Validate a single image file. Returns True if valid."""
    ok = True
...
def main() -> None:
    if len(sys.argv) < 2:
        print("Usage: validate_images.py <file1.png> [file2.png ...]")
        sys.exit(1)

    files = sys.argv[1:]
    results = [validate_image(f) for f in files]
